...
public:
    ...
    virtual void copyMemberInImpl(
        const EXTERNAL *source, CELL *target, std::size_t num, MEMBER CELL:: *memberPointer) = 0;

    /**
     * Extract a streak of members from a streak of cells.
     */
    virtual void copyMemberOutImpl(
        const CELL *source, EXTERNAL *target, std::size_t num, MEMBER CELL:: *memberPointer) = 0;
