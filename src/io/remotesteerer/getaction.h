...
{
public:
    explicit GetAction(const std::string& memberName) :
        PassThroughAction<CELL_TYPE>(
            "get_" + memberName,
            "usage: \"get_" + memberName +
            " T X Y [Z] MEMBER\", will return member MEMBER at time step T of cell"
            "at grid coordinate (X, Y, Z) if the model is 3D, or (X, Y) in the 2D case")
    {}
};
