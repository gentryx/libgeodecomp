...
public:
    ...
    {
        FixedArray<int, 20> a;
        TS_ASSERT_EQUALS(size_t(0), a.size());

        a << 0
          << 1
          << 2
          << 3;
        TS_ASSERT_EQUALS(size_t(4), a.size());
        ...
        a.erase(&a[1]);
        TS_ASSERT_EQUALS(size_t(3), a.size());
        TS_ASSERT_EQUALS(0, a[0]);
